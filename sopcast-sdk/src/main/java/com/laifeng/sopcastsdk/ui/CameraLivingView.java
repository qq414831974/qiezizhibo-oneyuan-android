...
//                    }
                    chooseVoiceMode();
                    screenOn();
                    mStreamController.start(mLivingStartListener);
                } else {
                    if (mLivingStartListener != null) {
                        mHandler.post(new Runnable() {
                            @Override
    ...
    public void restart() {
        mStreamController.restart(mLivingStartListener);
    }
    ...
    public int getMaxZoom() {
        return CameraHolder.instance().getCameraMaxZoom();
    }

    public void zoom(int value) {
        float progress = CameraHolder.instance().cameraZoom(value);
        onZoomProgress(progress);
    }

    public void zoom(boolean isBig, int scale) {
        float progress = CameraHolder.instance().cameraZoom(isBig, scale);
        onZoomProgress(progress);
    }
}
