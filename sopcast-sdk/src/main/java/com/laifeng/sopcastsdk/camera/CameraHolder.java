...
public class CameraHolder {
    ...
    public float cameraZoom(boolean isBig) {
        ...
        int zoom = 0;
        if (isBig) {
            zoom = Math.min(params.getZoom() + 1, params.getMaxZoom());
        } else {
            zoom = Math.max(params.getZoom() - 1, 0);
        }
        params.setZoom(zoom);
        mCameraDevice.setParameters(params);
    ...
    public float cameraZoom(boolean isBig, int scale) {
        if (mState != State.PREVIEW || mCameraDevice == null || mCameraData == null) {
            return -1;
        }
        Camera.Parameters params = mCameraDevice.getParameters();
    ...
    public float cameraZoom(int value) {
        if (mState != State.PREVIEW || mCameraDevice == null || mCameraData == null) {
            return -1;
        }
        Camera.Parameters params = mCameraDevice.getParameters();
        int zoom = Math.min(value, params.getMaxZoom());
        params.setZoom(zoom);
        mCameraDevice.setParameters(params);
        return (float) params.getZoom() / params.getMaxZoom();
    }
