...
public class RenderScreen {
    ...
    public void setWatermark(Watermark watermark) {
        ...
        mWatermarkImg = watermark.getMarkImg();
        initWatermarkVertexBuffer();
    }
    ...
    private void initWatermarkVertexBuffer() {
        ...
        int width = (int) (mWatermark.getWidth() * mWatermarkRatio);
        int height = (int) (mWatermark.getHeight() * mWatermarkRatio);
        int vMargin = (int) (mWatermark.getvMargin() * mWatermarkRatio);
        int hMargin = (int) (mWatermark.gethMargin() * mWatermarkRatio);
        if (mWatermark.isFullScreen()) {
            width = mScreenW;
            height = mScreenH;
        }

        boolean isTop, isRight;
        if (mWatermark.getOrientation() == WatermarkPosition.WATERMARK_ORIENTATION_TOP_LEFT
                || mWatermark.getOrientation() == WatermarkPosition.WATERMARK_ORIENTATION_TOP_RIGHT) {
            isTop = true;
        } else {
            isTop = false;
        }

        if (mWatermark.getOrientation() == WatermarkPosition.WATERMARK_ORIENTATION_TOP_RIGHT
                || mWatermark.getOrientation() == WatermarkPosition.WATERMARK_ORIENTATION_BOTTOM_RIGHT) {
            isRight = true;
        } else {
            isRight = false;
        }
