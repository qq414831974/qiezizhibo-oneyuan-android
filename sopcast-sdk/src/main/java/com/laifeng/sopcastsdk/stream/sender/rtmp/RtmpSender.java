...
public class RtmpSender implements Sender, SendQueueListener {
    ...
    @Override
    public void debug(final DebugInfo debugInfo) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
                    mListener.onDebug(debugInfo);
                }
            }
        });
    }
    ...
    public interface OnSenderListener {
        ...
        void onDebug(DebugInfo debugInfo);
    }
    ...
    public void setVideoParams(int width, int height, int rate, int fps) {
        rtmpConnection.setVideoParams(width, height, rate, fps);
    }

    public void setVideoParams(int width, int height, int fps) {
        setVideoParams(width, height, 0, fps);
    }
    ...
    @Override
    public void onVideoBpsChange(int bps) {
        rtmpConnection.setVideoRate(bps);
    }
