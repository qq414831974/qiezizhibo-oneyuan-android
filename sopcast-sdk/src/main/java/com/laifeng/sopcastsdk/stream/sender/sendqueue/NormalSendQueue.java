...
public class NormalSendQueue implements ISendQueue {
    ...
    private ArrayBlockingQueue<Frame<Chunk>> mFrameBuffer;
    ...
    @Override
    public SendQueueListener getSendQueueListener() {
        return mSendQueueListener;
    }
    ...
    @Override
    public void putFrame(Frame<Chunk> frame) {
    ...
    private void abandonData() {
        if (mTotalFrameCount.get() >= (mFullQueueCount / 2)) {
            ...
            boolean start = false;
            for (Frame<Chunk> frame : mFrameBuffer) {
    ...
    private class ScanThread extends Thread {
        private static final int SCAN_MAX_TIME = 4;
